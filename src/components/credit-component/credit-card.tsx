...
const creditCard: FC<Props> = ({ id, name, job, image }) => {
  return (
    <Link
      to={`/person/${id}`}
      className="group w-[12.16rem] h-[26.06rem] border-2 border-black mb-2 hover:border-primary transition-all duration-500 ease-in  rounded-[1.25rem] overflow-hidden shadow-lg flex-shrink-0 text-primaryText"
    >
      <img
